...
2. **ALWAYS discover files with available search tools first.** Prefer `rg` for discovery; fall back to `find` + `grep` before ANY edits. Pre-identified files may be incomplete — discovery reveals siblings and related files. Never skip this step.
