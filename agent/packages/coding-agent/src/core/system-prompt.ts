/**
 * oba-v27 — prompt builder with ripgrep-accelerated file discovery.
 */
...
function buildTaskDiscoverySection(taskText: string, cwd: string): string {
	try {
		...
		// oba-v27: prefer ripgrep (`rg`) for task discovery because it's fast and supports
		// reliable glob include/exclude. Fall back to grep if `rg` is unavailable.
		const isWin = process.platform === "win32";
		const nullSink = isWin ? "2>nul" : "2>/dev/null";
		const includeGlobsRg = isWin
			? '--glob "*.ts" --glob "*.tsx" --glob "*.js" --glob "*.jsx" --glob "*.mjs" --glob "*.cjs" --glob "*.py" --glob "*.go" --glob "*.rs" --glob "*.java" --glob "*.kt" --glob "*.scala" --glob "*.dart" --glob "*.rb" --glob "*.cs" --glob "*.cpp" --glob "*.c" --glob "*.h" --glob "*.hpp" --glob "*.vue" --glob "*.svelte" --glob "*.css" --glob "*.scss" --glob "*.html" --glob "*.json" --glob "*.yaml" --glob "*.yml" --glob "*.toml" --glob "*.md"'
			: "--glob '*.ts' --glob '*.tsx' --glob '*.js' --glob '*.jsx' --glob '*.mjs' --glob '*.cjs' --glob '*.py' --glob '*.go' --glob '*.rs' --glob '*.java' --glob '*.kt' --glob '*.scala' --glob '*.dart' --glob '*.rb' --glob '*.cs' --glob '*.cpp' --glob '*.c' --glob '*.h' --glob '*.hpp' --glob '*.vue' --glob '*.svelte' --glob '*.css' --glob '*.scss' --glob '*.html' --glob '*.json' --glob '*.yaml' --glob '*.yml' --glob '*.toml' --glob '*.md'";
		const excludeGlobsRg = isWin
			? '--glob "!node_modules/**" --glob "!.git/**" --glob "!dist/**" --glob "!build/**" --glob "!out/**" --glob "!.next/**" --glob "!target/**"'
			: "--glob '!node_modules/**' --glob '!.git/**' --glob '!dist/**' --glob '!build/**' --glob '!out/**' --glob '!.next/**' --glob '!target/**'";
